...
function renderFactorySyncStatus(factory) {
  ...
  if (sync.ok) {
    ...
    const count = sync.rowCount ?? factory.rowCount ?? 0;
    return `<div class="factory-sync-status ok">已同步 ${escapeHtml(count)} 条记录${time ? ` · ${escapeHtml(time)}` : ""}</div>`;
  }
...
function bindFarmActions() {
  ...
  document.querySelectorAll("[data-factory-bind-form]").forEach((factoryForm) => {
    factoryForm.addEventListener("submit", async (event) => {
      ...
      const sync = result?.todoFactorySync;
      if (sync?.ok) {
        showToast(`底库已绑定并同步：${sync.rowCount ?? 0} 条记录，新增 ${sync.createdFields ?? 0} 个字段`);
      } else if (sync?.message) {
        showToast(`底库已绑定，但同步失败：${sync.message}`);
      } else {
        showToast("多维表格底库已绑定");
      }
    });
